...
ACL.prototype.createRole = function createRole (data, cb){
  ...
  if (this.app.config.autoUpdateRolesConfig) {
    // then write to file
    this.writeRolesToConfigFile(function afterSaveRoles (err){
      cb(err, self.roles[data.name]);
    });
  } else {
    cb(null, this.roles[data.name]);
  }
};
...
ACL.prototype.deleteRole = function deleteRole (roleName, cb){
  ...
  // then write to file
  if (this.app.config.autoUpdateRolesConfig) {
    this.writeRolesToConfigFile(cb);
  } else {
    cb();
  }
};
...
ACL.prototype.canMiddleware = function canMiddleware (req, res, callback){
  ...
  acl.loadUserContextRoles(req, res, function afterLoadUserRoles (err){
    ...
    if (res.locals.isAdmin) {
      ...
      if (!acl.canStatic('access_admin', req.userRoleNames))
    ...
    if (!acl.canStatic(res.locals.permission, req.userRoleNames)) {
      // cant
      if (!req.user) {
        acl.app.log.info('ACL:canMiddleware: forbidden for unAuthenticated user:', res.locals.permission);
      } else {
        acl.app.log.info('ACL:canMiddleware: forbidden for user id: ',
